// ===== DAVID UNTERBERGER TERMINAL - COMPLETE WITH ALL FIXES =====
...
document.documentElement.style.setProperty('--terminal-opacity', '1');
    ...
    <div class="cli-container">
      ...
      <div class="cli-input-line">
        ...
        <div class="cli-input-wrapper">
          <input type="text" class="cli-input" id="cli-input" autocomplete="off" />
          <span class="cli-cursor"></span>
          <span class="cli-placeholder"></span>
        </div>
      </div>
      <div class="section-footer">╚════════════════════════════════════════════════════════════════════════════════════════════════════╝</div>
    </div>
...
function initCLI() {
  ...
  function handleTabCompletion(input) {
    ...
    const allCommands = [
      ...
      'quiz', 'riddle', 'fact', 'challenge', 'ip', 'whoami', 'opacity',
      'clear-history', 'ls', 'cd', 'pwd', 'cat', '?', 'quit'
    ];
  ...
  function executeCommand(cmd) {
    ...
    } else if (cmd === 'rm -rf' || cmd.startsWith('rm -rf ')) {
      output.className = 'cli-output cli-error';
...
To add the full lyrics:
1. Open terminal.js
2. Find this command (search for "banküberfall")
3. Replace this text with your song lyrics

═══════════════════════════════════════════════════

Austrian comedy song by EAV (Erste Allgemeine Verunsicherung)`;

    } else if (cmd.toLowerCase() === 'da hofa woas' || cmd.toLowerCase() === 'der hofer wars' || cmd.toLowerCase() === 'da hofa') {
      output.innerHTML = `🎭 Der Hofer War's! 🎵
...
To add the full lyrics:
1. Open terminal.js
2. Find this command (search for "da hofa")
3. Replace this text with your song lyrics
...
:wq / quit      - Try to leave
teapot          - HTTP 418
banküberfall    - Austrian bank robbery song
da hofa woas    - Austrian folk song
meep            - Echo meep
...
opacity [0-100] - Set terminal transparency
...
Navigation: discord, steam, github, itch, weather, nasa, news`;
          break;
